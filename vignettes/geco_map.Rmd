...
The above workflow is now adopted as `plot_map4()` in the rbeni package (as is `nc_to_df` which reads a NetCDF file into a data frame).
```{r}
library(rbeni)

df <- nc_to_df("../data/cwdx80_halfdeg.nc", varnam = "cwdx80") |> 
  mutate(lon = round(lon, digits = 2), lat = round(lat, digits = 2))
...
This follows the example from Koen Hufken's [blog post](https://bluegreenlabs.org/post/map-building-3/).
...
rasta <- raster("../data/cwdx80_halfdeg.nc")
...
# re-project and convert gridded raster dato dataframe
df <- projectRaster(rasta, crs = robinson) |> 
  rasterToPoints() |> 
  as_tibble()  |> 
  `colnames<-`(c("x", "y", "cwdx80"))
...
- coastline is not showing.
- Plotting outside the panel 
